...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (page === "store") {
    ...
    wireValidation(bodyEl, saveBtn); // Wire validation initially

    const checkExistingAttendance = async () => {
        const subject_id = subjSel.value;
        if (!subject_id) {
            bodyEl.innerHTML = ""; // Clear table if no subject selected
            statusMsg.innerHTML = "";
            wireValidation(bodyEl, saveBtn); // Re-validate to disable button
            return;
        }
        ...
        if (data.ok && data.records) {
            ...
            if (attendanceTaken) {
                statusMsg.innerHTML = "✅ Attendance already taken for this subject. You can edit the records below.";
                statusMsg.style.color = "green";
            } else {
                statusMsg.innerHTML = "ℹ️ Attendance for this subject has not been taken yet today.";
                statusMsg.style.color = "#333";
            }
            wireValidation(bodyEl, saveBtn); // Re-validate after checking radios
        }
    };
  ...
  if (page === "individual") {
    ...
    // Change the default option text
    ...
    btn.addEventListener("click", async () => {
      ...
      if (!query) {
        info.innerHTML = "<p>Please enter a name or roll number to search.</p>";
        rep.innerHTML = "";
        return;
      }
